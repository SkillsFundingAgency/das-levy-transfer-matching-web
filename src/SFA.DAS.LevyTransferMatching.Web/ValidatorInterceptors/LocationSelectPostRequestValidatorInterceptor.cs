...
public class LocationSelectPostRequestValidatorInterceptor : IValidatorInterceptor
{
    public ValidationResult AfterAspNetValidation(ActionContext actionContext, IValidationContext validationContext, ValidationResult result)
    {
        ...
        for (var index = 0; index < postRequest.SelectValidLocationGroups.Length; index++)
        {
            placeholderModelStateValues.SetModelValue($"{nameof(LocationSelectPostRequest.SelectValidLocationGroups)}[{index}]", string.Empty, string.Empty);
            placeholderModelStateValues.MarkFieldValid($"{nameof(LocationSelectPostRequest.SelectValidLocationGroups)}[{index}]");
        }
