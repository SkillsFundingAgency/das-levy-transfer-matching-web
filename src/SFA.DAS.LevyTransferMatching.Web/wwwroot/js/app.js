...
// Opportunities/Sector.cshtml - clear the AdditionalLocationText input value if the AdditionalLocation checkbox is unchecked
// to prevent checkbox from being re-checked if the modelstate is invalid in postback as the text input has a value.
const sectorAdditionalLocationCheckBox = document.getElementById('AdditionalLocation');

if (sectorAdditionalLocationCheckBox) {
    sectorAdditionalLocationCheckBox.addEventListener("change", function () {
        if (!sectorAdditionalLocationCheckBox.checked) {
            const sectorAdditionalLocationTextInput = document.getElementById('AdditionalLocationText');
            if (sectorAdditionalLocationTextInput) {
                sectorAdditionalLocationTextInput.value = '';
            }
        }
    });
}
