    ...
    : IPledgeOrchestrator
{
    ...
    private IEnumerable<PageLink> BuildPageLinks(GetPledgesResponse pledgesResponse)
    {
        ...
        for (var index = 0; index < totalPageLinks; index++)
        {
            var link = new PageLink
            {
                ...
                RouteData = BuildRouteData(pageNumberSeed + index)
            };
            links.Add(link);
        }
    ...
    {
        var result = await pledgeService.GetApplications(request.AccountId, request.PledgeId, request.SortColumn, request.SortOrder);
