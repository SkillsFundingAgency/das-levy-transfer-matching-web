...
namespace SFA.DAS.LevyTransferMatching.Infrastructure.Services.PledgeService
{
    public class PledgeService : IPledgeService
    {
        ...
        public async Task RejectApplications(RejectApplicationsRequest request, long accountId, int pledgeId)
        {
            var json = JsonConvert.SerializeObject(request, new StringEnumConverter());
        ...
        {
            var json = JsonConvert.SerializeObject(request);
            var response = await _client.PostAsync($"accounts/{accountId}/pledges/{pledgeId}/applications/{applicationId}/approval-options", new StringContent(json, System.Text.Encoding.UTF8, "application/json"));
            response.EnsureSuccessStatusCode();
        }
    }
}